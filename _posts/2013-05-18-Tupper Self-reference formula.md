...
![](/media/img/2013/Self-reference formula.png)
...
公式的核心原理懂了，那里面的17又是什么意思呢？仔细观察图片，17实际上就是可控图像的宽度，而\\( N \\)的值就等于\\( 17*图像的二进制表达式 \\)。所以真实情况是\\( N的值是在图像确定后才算出来的 \\)。了解了公式的构造和\\( N \\)值的确定，理论上我们就可以在某个公式图像的某个位置找出任何我们想要的图像，只要我们确定图像的范围（\\( N \\)值）并同时确定公式就可以得到在指定公式下找到我们想要的图像。最后给出找出图像的通用公式（\\( M \\)为区分小数部分的标识符，\\( L,H \\)为图像的长、高）
...
最后，附上自我指涉的Mathematica程序地址：<https://github.com/AImager/blog_code/blob/master/2013/>
